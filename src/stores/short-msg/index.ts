...
const stmsgStore = defineStore('short-msg', {
  state: () => ({
    loading: false,
    shortMessageList : [] as ShortMsgType[],
    groups: [] as GroupType[],
  ...
  actions: {
    ...
    async getShortMessages(
      ...
      try {
        ...
        if (res && !fun) {
          this.shortMessageList =
            // page == 1 ? res.data : this.shortMessageList.concat(res.data)
            page == 1 ? res.rows : this.shortMessageList.concat(res.rows)
